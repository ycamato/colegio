...
<body>
	...
	$sql_total="SELECT * FROM materias";//muestra los 3 primeros, primer parametro indica en que posición impieza en este caso posición 0, el segundo parametro cuantos registros quiere mostrar en este caso 3 registros
	...
	$registros=$base_de_datos->query("SELECT * from materias LIMIT $empieza, $registros")->fetchALL(PDO::FETCH_OBJ);
	...
	if(isset($_POST['inserta'])){
		...
		<?php 
		$sql="INSERT INTO materias (n_materias,nombre, horas) values ( :id,:mat ,:h)";
		$resultado=$base_de_datos->prepare($sql);//$base es el nombre de la conexión
		$resultado->execute(array( ":id"=>$id,  ":mat"=>$materias , ":h"=>$horas));
...
<h3 align="center">PANEL DE OPCIONES MATERIAS</h3>
<form action=" " method="post" autocomplete="off">
		<table align="center" border="" bordercolor="orange">
			...
			<tr>
				...
				<td><?php echo $materia->horas?></td>

				
					
			<td><a href="eliminar.php?id=<?php echo $materia->n_materias?> & mat=<?php echo $materia->nombre?>& hor=<?php echo $materia->horas?>"><input type="button" name="elimina" id="elimina" value="Eliminar"></a></td>
			<td><a href="editar.php?id=<?php echo $materia->n_materias?> & mat=<?php echo $materia->nombre?>& hor=<?php echo $materia->horas?>"><input type="button" name="edita" class="edita" value="Editar"></a></td></tr>
			...
			<td><input type="text" name="materias"></td>
			<td><input type="varchar" name="horas"></td>
